...
Claude Bridge gives Claude Code **167 MCP tools** across **16 categories**, enough
...
<!-- BEGIN TOOL TABLE -->
<!-- AUTO-GENERATED by scripts/generate_tool_doc.py — 167 tools. Do not edit by hand. -->
...
| `list_uncataloged_devices` | List plugin-owned device TYPES that have no profile in the vendored device catalogue — the gap report for keeping the capability catalogue current. Collapses duplicates: 19 Shelly plugs of one type report as a single uncatalogued type with a device_count and an example device. Built-in / interface devices (no pluginId) are excluded. |
...
│           └── tools/                      # 19 tool handler modules (167 tools)
